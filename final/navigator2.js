...
fEmpty=loadImage('graphic/button_final_empty.png');
...
fFilled=loadImage('graphic/button_final_filled.png');
...
function draw(){
  ...
  if(sessionStorage.b1=="true"){
    if(sessionStorage.b2=="true"){
      if(sessionStorage.b3=="true"){
        buttonF(500,500);
      } else { return false; }
    } else { return false; }
  } else { return false; }
...
function buttonF(_x, _y){
  ...
  rectMode(CORNER);
  if(fillingF<200){
    rect(x,y+200-fillingF,200,fillingF);
  } else {
    rect(x,y,200,200);
  }
  ...
  if (fillingF>200){
    image(fFilled,x,y);
    noStroke();
    fill(47,255,47,255-(fillingF-200));
    rect(x,y,200,200);
  }
